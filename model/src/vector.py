...
def vector_to_string(vector, should_print=False):
    results = []

    if should_print:
        print("vector global:", vector)
    ...
    if vector.ndim == 2:  # Batch processing
        for vec in vector:
            result = process_single_vector(vec, should_print=should_print)
            results.append(result)
    else:  # Single vector processing
        result = process_single_vector(vector, should_print=should_print)
        results.append(result)

    # Return results based on whether it was a batch or a single vector
    return results if len(results) > 1 else results[0]

def process_single_vector(vector, should_print=False):
    if should_print:
        print("vector:", vector)
        print("type(vector):", type(vector))
